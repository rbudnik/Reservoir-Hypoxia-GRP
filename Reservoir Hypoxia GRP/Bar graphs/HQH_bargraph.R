...
# Create jpeg of plot
jpeg(filename = paste("C:/Users/richa/OneDrive/Desktop/HQH_bargraph.jpeg", sep = ""),
     width = 7.3, height = 3, units = "in", res = 600, restoreConsole = TRUE)


# Largemouth Bass Plot
LMBplot <- ggplot(data1, aes(fill=fcDO, y=HQH, x=Reservoir)) + 
              ...
              annotate("text", x = .65, y = 94.5, label = "(A)", size = 4.5) +
              labs(y = "High Quality Habitat (%)")

             
SAEplot <- ggplot(data2, aes(fill=fcDO, y=HQH, x=Reservoir)) + 
              ...
              axis.line = element_line(colour = "black")) +
              geom_hline(yintercept = 0) + 
              annotate("text", x = .65, y = 94.5, label = "(B)", size = 4.5)

WHCplot <- ggplot(data3, aes(fill=fcDO, y=HQH, x=Reservoir)) + 
              geom_bar(position="dodge", colour="black", stat="identity", show.legend = FALSE) +
              scale_y_continuous(expand = c(0,0)) +
              scale_fill_manual(values=c("white", "gray63")) +
              coord_cartesian(ylim = c(0, 100)) +
              ...
              axis.line = element_line(colour = "black")) +
              geom_hline(yintercept = 0) +
              annotate("text", x = .65, y = 94.5, label = "(C)", size = 4.5)
